                ...
                if ((tipo_transaccion && id_transaccion) || tranid_Array.length > 0) {
                    ...
                    if (!tran_uuid) {
                        ...
                        try {
                            ...
                            log.audit({title:'TIPO DE  TRANSACCION',details:tipo_transaccion});
                            if(tipo_transaccion=='customerpayment'){
                                log.audit({title:'ENTRADA AL IF🛺🛺',details:tipo_transaccion});
                                     //AQUI VA PAGO GLOBAL CON ADDCUSTOMDATASOURCE
                                // Obtener el ID de la factura global
                                var idFacturaGlobal = recordObjrecord.getValue('custbody_efx_fe_id_facturaglobal');
    
                                // Definir la función para obtener los datos de la factura global
                               function fetchFGData(idFacturaGlobal) {
                                ...
                                var data = {
                                    ...
                                    currency: '',
                                        ...
                                        if (idFacturaGlobal) {
                                            // Lógica para buscar la factura global utilizando el operador "anyof"
                                            var FacturaGlobalSearch = search.create({
                                                type: "customsale_efx_fe_factura_global",
                                                filters: [
                                                    ["internalid", "anyof", idFacturaGlobal],
                                                ],
                                                columns: [
                                            ...
                                            if (result) {
                                                ...
                                                log.audit({ title: 'DATOS DE FETCHFG DATA👌👌👌', details: data });
                                            } else {
                                                log.error({ title: 'Error', details: 'No se encontró la factura global con el ID proporcionado' });
                                            }
                                        } 
                                ...
                                // Obtener los datos de la factura global
                                var data = fetchFGData(idFacturaGlobal);
                                log.audit({title:'DATOS DE FETCHFG DATA👌👌👌',details:data});
                                // Agregar los datos a la plantilla FTL
